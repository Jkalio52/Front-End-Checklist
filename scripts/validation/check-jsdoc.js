...
const REQUIRED_TAGS = {
  functions: [], // Description-only is sufficient; TypeScript provides param/return types
  components: [], // Description-only; props are typed via TypeScript interfaces
  classes: [], // Classes just need description
  hooks: [] // Description-only; TypeScript provides param/return types
}
...
const EXCLUDE_PATTERNS = [
  ...
  /packages\/mcp\/src\/tools\/get-checklist-rules\.ts$/,
  /packages\/mcp\/src\/tools\/get-quick-reference\.ts$/,
...
/**
 * Check whether a file should be excluded from validation.
...
/**
 * Check whether a directory should be excluded from validation.
 * @param {string} dirPath - Directory path to inspect.
 * @returns {boolean} True when the directory is excluded.
 */
...
/**
 * Validate JSDoc coverage for staged source files.
 */
class JSDocValidator {
  ...
  validateFile(filePath) {
    ...
    lines.forEach((line, index) => {
      ...
      patterns.forEach(pattern => {
        this.processPatternMatch(pattern, trimmedLine, index, content, filePath)
      })
    })
  }
   ...
   */
  processPatternMatch(pattern, trimmedLine, index, content, filePath) {
  ...
  validateDirectory(dir) {
    ...
    items.forEach(item => {
      ...
      if (stat.isDirectory()) {
        ...
        if (this.shouldExcludeFile(fullPath)) return
...
function main() {
  ...
  if (specificFiles.length > 0) {
    ...
    specificFiles.forEach(file => {
      if (fs.existsSync(file) && !validator.shouldExcludeFile(file)) {
